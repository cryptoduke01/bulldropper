...
export function ConnectWalletButton({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1.5 rounded-full border border-[color:var(--color-border)] bg-[color:var(--color-bg-elev)] py-1 pr-1 pl-2.5 text-xs sm:text-[13px]">
      {wallet?.adapter.icon && (
        <img src={wallet.adapter.icon} alt="" width={13} height={13} className="rounded-sm" />
      )}
      <span className="font-mono tabular text-[color:var(--color-fg)]">
        {shortAddress(addr, compact ? 3 : 3, 3)}
      </span>
      <button
        onClick={() => disconnect()}
        className="rounded-full bg-[color:var(--color-bg-elev-2)] px-2 py-0.5 text-[10px] sm:text-[11px] text-[color:var(--color-fg-muted)] transition hover:text-[color:var(--color-fg)]"
      >
        ×
      </button>
    </div>
